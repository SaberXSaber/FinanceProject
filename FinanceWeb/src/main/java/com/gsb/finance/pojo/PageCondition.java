...
public class PageCondition {
   ...
   /* { oper:'eq', text:'等于\u3000\u3000'},{ oper:'ne', text:'不等\u3000\u3000'},
    ...
    private String jsonFilter(String filters ){

        JSONObject jsonFilter = (JSONObject)JSONObject.parseObject(filters);
        String groupOp = jsonFilter.getString("groupOp");
        JSONArray rules = jsonFilter.getJSONArray("rules");
        String result= "";
        for (int i = 0; i < rules.size(); i++) {
            JSONObject rule = rules.getJSONObject(i);
            result =result+ " "+groupOp +" "+getsql(rule.getString("field"),rule.getString("op"),rule.getString("data"));
        }
        return result;
    }
}
